...
void pair_gpu(double const *d_x, double const *d_y, double const *d_z,
			  std::atomic<unsigned long long> *d_g2, int numatm, int nconf,
...
int main(int argc, char *argv[])
{
	...
	std::vector<std::atomic<unsigned long long>> h_g2_vector(nbin);
	std::atomic<unsigned long long> *h_g2 = h_g2_vector.data();
	//std::fill_n(std::execution::par,h_g2_vector.begin(), nbin, 0);
	...
	for (int i = 0; i < nbin; i++)
	{
		...
		double box = std::min({xbox, ybox, zbox});
			  ...
			  const double xbox, const double ybox, const double zbox, int d_bin)
{
	...
	for (int frame = 0; frame < nconf; frame++)
	{
		...
		std::Fill parallel algorithm Here(Fill execution policy here, res.begin(), res.end(),
